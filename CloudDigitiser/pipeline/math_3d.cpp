...
float Vector3f::Magnitude2()
{
    return (x * x + y * y + z * z);
}

void Vector3f::Rotate(float degrees, const Vector3f& Axe)
{
    const float SinHalfAngle = sinf(ToRadian(degrees/2));
    const float CosHalfAngle = cosf(ToRadian(degrees/2));
...
void Matrix4f::InitQuaternion(const Quaternion& q)
{
    float sqw = q.w*q.w;
    float sqx = q.x*q.x;
    float sqy = q.y*q.y;
    ...
    // set values for last row and column
...
void Matrix4f::InitOrthoProjTransform(const PersProjInfo& p)
{
    ...
    m[2][0] = 0.0f;                 m[2][1] = 0.0f;              m[2][2] = -2.0f/(p.zFar-p.zNear); m[2][3] = ((p.zFar+p.zNear)/(p.zFar-p.zNear));
    m[3][0] = 0.0f;                 m[3][1] = 0.0f;              m[3][2] = 0.0f;                   m[3][3] = 1.0f;
}
...
Quaternion MakeQuaternionFromAxisAngle(float x, float y, float z, float degrees)
{
    ...
    float sin_a = sinf( radians / 2 );
    float cos_a = cosf( radians / 2 );

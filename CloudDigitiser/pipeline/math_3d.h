...
struct Vector3f
{
    ...
    Vector3f(float _x, float _y, float _z, float _w)
    {
        x = _x / _w;
        y = _y / _w;
        z = _z / _w;
    ...
    void Print() const
    {
        fprintf(stderr, "(%.02f, %.02f, %.02f)", x, y, z);
    }
};
...
struct Vector4f
{
    ...
    void Print() const
    {
        fprintf(stderr, "(%.02f, %.02f, %.02f, %.02f)", x, y, z, w);
    }
};
...
Quaternion MakeQuaternionFromAxisAngle(float x, float y, float z, float degrees);

Vector3f QVRotate(const Quaternion& q, const Vector3f& v);
...
public:
    ...
    void InitQuaternion(const Quaternion& q);
};
